...
from periodic_shared import structure_panel as sp  # noqa: E402
...
def test_the_panel_hands_the_drawn_molecule_back_as_a_template(qapp):
    """Previewing the user's molecule must not replace it with a rebuilt one."""
    mol = _FakeMol(["O", "H"], [[0.0, 0.0, 0.0], [0.96, 0.0, 0.0]])
    panel = sp.StructurePanel(get_molecule=lambda: mol)
    panel.source_combo.setCurrentText(sp.SOURCE_MOLECULE)
    cell = panel.build_cell()
    assert panel.source_template(cell) is mol
    panel.deleteLater()


def test_no_template_for_a_crystal(qapp, tmp_path):
    path = tmp_path / "x.cif"
    path.write_text(CUBIC_CIF, encoding="utf-8")
    panel = sp.StructurePanel()
    panel.source_combo.setCurrentText(sp.SOURCE_CIF)
    panel.cif_edit.setText(str(path))
    assert panel.source_template(panel.build_cell()) is None
    panel.deleteLater()


def test_no_template_when_the_supercell_changed_the_atom_count(qapp):
    mol = _FakeMol(["O", "H"], [[0.0, 0.0, 0.0], [0.96, 0.0, 0.0]])
    panel = sp.StructurePanel(get_molecule=lambda: mol)
    panel.source_combo.setCurrentText(sp.SOURCE_MOLECULE)
    panel.repeat_spins[0].setValue(2)
...
def test_no_template_without_a_molecule_getter(qapp):
    panel = sp.StructurePanel()
    assert panel.source_template(None) is None
    panel.deleteLater()
