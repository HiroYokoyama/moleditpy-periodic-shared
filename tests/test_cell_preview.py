...
from periodic_shared import cell_model as cm  # noqa: E402
from periodic_shared import cell_preview as cp  # noqa: E402


def test_shared_module_identity():
    assert cp.SHARED_MODULE_NAME == "periodic-cell-preview"
    assert cp.SHARED_MODULE_VERSION == "0.6.0"
...
# -- keeping what the user drew ---------------------------------------------


def _drawn_molecule():
    """Ethene: a double bond that a rebuilt molecule would lose."""
...
def test_boxing_a_molecule_keeps_its_double_bond():
    """Previewing must not quietly downgrade the structure being edited."""
    Chem = pytest.importorskip("rdkit.Chem")
    drawn = _drawn_molecule()
    conformer = drawn.GetConformer()
    coords = [list(conformer.GetAtomPosition(i)) for i in range(drawn.GetNumAtoms())]
    boxed = cm.cell_from_molecule(
        [a.GetSymbol() for a in drawn.GetAtoms()], coords, padding=5.0
    )
    shown = cp.build_molecule(boxed, template=drawn)
    orders = sorted(str(b.GetBondType()) for b in shown.GetBonds())
    assert "DOUBLE" in orders
    assert shown.GetNumBonds() == drawn.GetNumBonds()


def test_the_template_positions_follow_the_box():
    pytest.importorskip("rdkit")
    drawn = _drawn_molecule()
    conformer = drawn.GetConformer()
    coords = [list(conformer.GetAtomPosition(i)) for i in range(drawn.GetNumAtoms())]
    ...
    shown = cp.build_molecule(boxed, template=drawn)
    moved = shown.GetConformer()
    for index, atom in enumerate(boxed.atoms):
        position = moved.GetAtomPosition(index)
        assert (position.x, position.y, position.z) == pytest.approx(tuple(atom.cart))


def test_a_template_with_the_wrong_atom_count_is_ignored(cell):
    """A supercell has more atoms than the molecule it came from."""
    pytest.importorskip("rdkit")
    from rdkit import Chem

    wrong = Chem.MolFromSmiles("C")
    assert cp.build_molecule(cell, template=wrong).GetNumAtoms() == len(cell.atoms)


def test_a_template_that_cannot_be_copied_is_ignored(cell):
    pytest.importorskip("rdkit")
    ...
    assert cp.build_molecule(cell, template=Hostile()).GetNumAtoms() == len(cell.atoms)
